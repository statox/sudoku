import { recomputeAllNotes } from './grid';
...
export const solveGrid = (grid: Grid): Grid => {
    const gridToSolve = JSON.parse(JSON.stringify(grid));
    solveGridWFC(gridToSolve);
    return gridToSolve;
};

export const solveGridWFC = (grid: Grid) => {
    ...
    if (!gridIsValid(grid)) {
        console.log('Couldnt solve grid');
    }
};
...
export const wfcSolveStep = (grid: Grid) => {
